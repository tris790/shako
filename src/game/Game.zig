...
pub fn init(self: *@This()) void {
    ...
    player_transform_component.* = TransformComponent{ .position = c.Vector2{ .x = 50, .y = 50 }, .scale = c.Vector2{ .x = 25, .y = 25 } };
    var player_movement_component = self.ecs.getComponent(MovementComponent, 0);
    player_movement_component.* = MovementComponent{ .velocity = c.Vector2{ .x = 10, .y = 10 } };
    ...
    monster_transform_component.* = TransformComponent{ .position = c.Vector2{ .x = 200, .y = 200 }, .scale = c.Vector2{ .x = 75, .y = 75 } };
